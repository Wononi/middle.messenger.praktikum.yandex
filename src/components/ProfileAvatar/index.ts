...
export class ProfileAvatar extends Block<ProfileAvatarProps> {
  ...
  init() {
    this.element?.classList.add(s.avatar)
  ...
  render() {
    if (this.props.isAvatar) {
      const template = Handlebars.compile(`{{{img}}}`)
      return this.compile(template, this.props)
    } else {
      const template = Handlebars.compile(`<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
